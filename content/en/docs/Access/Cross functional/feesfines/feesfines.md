...
In FOLIO, the underlying account and action data would look like this:
...
The loaning service point calendar - in this example, let’s suppose that service point is open 24/7
...
It rounds the number of overdue time “units” up to the next integer - in this case, 2.
It multiplies the number of overdue time “units” by the amount of the fine per interval - 3 - to get a total fine of $6.00
...
The factors that are used in the overdue fine calculation include:
The loaning service point calendar - in this example, the service point is open 8 AM to midnight from May 1st to May 11th.
The stated overdue charge, defined in the overdue policy - in this example, $3/day
...
So FOLIO subtracts the closed minutes from the total overdue period - 3720 - 1440 = 2280 minutes. That is the amount of overdue time FOLIO will charge for.
...
GET /accounts?query=itemId=="_insert item UUID_"&limit=1000
...
* 9:00 PM - the scheduled-aged-to-lost process begins, there is no work to do
* 9:05 PM - item goes past the lost time frame and is able to be aged to lost
* 9:05 PM - the scheduled-aged-to-lost-fee-charging process begins, there is no work to do
* 9:30 PM - the scheduled-aged-to-lost process begins and changes the item status to aged to lost
* 9:35 PM - the scheduled-aged-to-lost-fee-charging process begins, takes in the aged to lost loan information, and applies the $75 charge.
