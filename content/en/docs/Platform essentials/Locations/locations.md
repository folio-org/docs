...
A floating collection is a collection of items that are not permanently housed at a specific library. Instead, items are shelved at the library where they were most recently checked in. When an item in a floating collection is checked in at another service point associated with a floating collection, the item's effective location is automatically updated to that service point's floating collection location.
